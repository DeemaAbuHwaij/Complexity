  ...
  let rec tag_parse sexpr =
    ...
    | ScmPair (ScmSymbol "letrec", ScmPair (ribs, exprs)) ->
      ...
      | ribs_list, ScmNil ->
        ...
        tag_parse(ScmPair(ScmSymbol("let"), ScmPair(scheme_sexpr_list_of_sexpr_list new_ribs, new_body)))
...
module Semantic_Analysis : SEMANTIC_ANALYSIS = struct
  ...
  let annotate_lexical_address pe =
    ...
    let rec run pe params env = 
      ...
      | ScmApplic (proc, args) ->ScmApplic' (run proc params env,List.map (fun arg -> run arg params env) args, Non_Tail_Call)
    in
      run pe [] [];;
  ...
  let annotate_tail_calls pe = 
    ...
    let rec run pe in_tail =
      ...
      | ScmConst'(_) -> pe
      | ScmVarGet' (_) -> pe
      | ScmVarDef'(vr, vl) -> ScmVarDef'(vr, (run vl false))
      | ScmIf'(test, if_t, if_f) -> ScmIf'((run test false), (run if_t in_tail), (run if_f in_tail))
      ...
      | ScmLambda'(args,Simple, body) -> ScmLambda'(args, Simple, run body true)
      | ScmLambda'(must, Opt option, body) -> ScmLambda'(must, Opt option, run body true)
      | ScmApplic'(proc, args, listApp) ->
          if in_tail 
    ...
    and runl in_tail expr = function
      | [] -> [run expr in_tail]
      | expr' :: exprs -> run expr in_tail :: runl in_tail expr' exprs
    in
      run pe false;;
    ...
    let collect_free_vars pe =
      let rec run pe params env = 
        match pe with
          ...
          | ScmVarGet' _ -> []
          | ScmIf' (test, dit, dif) -> (run test params env) @ (run dit params env) @ (run dif params env)
          | ScmSeq' exprs' -> runs exprs' params env
          | ScmOr' exprs' -> runs exprs' params env
          | ScmVarSet' (Var' (v, Free), expr') -> v :: (run expr' params env)
          | ScmVarSet' (_, expr') -> run expr' params env
          | ScmVarDef' (Var' (v, Free), expr') -> v :: (run expr' params env)
          | ScmVarDef' (_, expr') -> run expr' params env
          | ScmBox' (Var' (v, Free)) -> [v]
          | ScmBox' _ -> []
          | ScmBoxGet' (Var' (v, Free)) -> [v]
          | ScmBoxGet' _ -> []
          | ScmBoxSet' (Var' (v, Free), expr') -> v :: (run expr' params env)
          | ScmBoxSet' (_, expr') -> run expr' params env
          | ScmLambda' (_, _, expr') -> run expr' params env
          | ScmApplic' (expr', exprs', _) -> (run expr' params env) @ (runs exprs' params env)
        and runs exprs' params env = 
          List.fold_left
          (fun vars expr' -> vars @ (run expr' params env))
          []
          exprs'
        in
          let primitives = 
